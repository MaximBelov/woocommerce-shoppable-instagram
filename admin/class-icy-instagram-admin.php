...
class WC_Shoppable_Instagram_Admin {
	...
	public function admin_scripts() {
		...
		wp_enqueue_script( 'icywp-instagram-admin', plugin_dir_url( __FILE__ ) . '/assets/js/icy-instagram-admin.js', array( 'jquery' ), time(), true );
	...
	public function updateAuth() {
		...
		if( isset( $_GET['token'] )
		    && get_option( 'icy_instagram_feed_token', '0' ) == '0'
		) {
				...
				<p>
					...
					<a href="<?php print
						esc_url(
							add_query_arg(
								array(
									'client_id'     => $settings_value['api_key'],
									'redirect_uri'  => urlencode(
										admin_url( 'options-general.php?page=instagram-feed' )
									),
									'response_type' => 'token',
									'type'=>'web_server'
								),
					...
					?>" class="button-secondary connect"><?php _e( 'Authenticate with Instagram' ); ?></a>
							...
							<div class="ig-image" data-image-id="<?php print $entry->id; ?>">
								<div class="ig-img-container">
									<a href="#TB_inline?width=600&height=600&inlineId=ig-image-overlay" class="thickbox ig-img-settings-action">
										<img src="<?php print $entry->imageThumbnailUrl; ?>" />
									</a>
								</div>
								<div class="ig-img-data" data-image="<?php print $entry->imageStandardResolutionUrl; ?>"></div>
							</div>
