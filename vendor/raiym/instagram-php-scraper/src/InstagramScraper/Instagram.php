...
class Instagram
{
    ...
    public function getPaginateMediasByTag($tag, $maxId = '')
    {
	    ...
	    $top_posts = $arr['tag']['top_posts']['nodes'];

	    if (count($top_posts) != 0) {
		    foreach ($top_posts as $mediaArray) {
	    ...
	    $medias = $this->my_array_unique_by_subkey($medias, 'id');
	...
	public function my_array_unique_by_subkey($array,$subkey){
