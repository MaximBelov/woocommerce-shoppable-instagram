...
class Icy_Instagram_Feed {
	...
	public function frontend_scripts() {
		...
		wp_enqueue_style( 'magnific-style', WCSI_PLUGIN_URL . '/assets/js/magnific-popup/dist/magnific-popup.css', array(), ICYIG_VER );
		wp_enqueue_script( 'magnific-popup', WCSI_PLUGIN_URL . '/assets/js/magnific-popup/dist/jquery.magnific-popup.js', array( 'jquery' ), ICYIG_VER, true );
				...
				foreach( $hotspots as $hsid => $hotspot ) {
					...
					if(false !== $product_post_type ) {
						if( in_array( $product_post_type, array( 'product', 'variation' ) ) ) {
							...
							$mdata[$image_id]['hotspots'][$hsid]['image'] = $image_src;
						}
					}
